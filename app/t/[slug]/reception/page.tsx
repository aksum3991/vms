...
function ReceptionPageContent() {
  ...
  const loadGuests = async () => {
    try {
      const [allRequests, fetchedSettings] = await Promise.all([getRequests(slug as string), getSettings(slug as string)])
      setSettings(fetchedSettings)
      
      const guestList: GuestWithRequest[] = []

      console.log("Reception: Fetched requests:", allRequests.length)
      console.log("Reception: User:", JSON.stringify(user))

      allRequests.forEach((request) => {
        // Debug each request processing
        console.log(`Processing Req ${request.id}: Status=${request.status}, Gate=${request.gate}, Guests=${request.guests.length}`)

        // Skip only withdrawn requests. 
        // We want to show ANY guest that is fully approved, even if the request itself is partially rejected or pending other things.
        if (request.status === "withdrawn") {
          console.log(`  -> Skipped: Withdrawn`)
          return
        }
        ...
        if (!shouldIncludeRequest) {
          console.log(`  -> Skipped: No gate access (User gates: ${user?.assignedGates}, Req gate: ${request.gate})`)
          return
        }
        ...
        if (approvedGuests.length === 0) {
           console.log("  -> Skipped: No approved guests")
        }
  ...
  const handleCheckIn = async (guestId: string, requestId: string) => {
    try {
      ...
      const result = await checkInGuest(requestId, guestId)
      ...
      // notification trigger might fail but shouldn't block UI success
      try {
  ...
  const performCheckOut = async () => {
    ...
    try {
      // Perform Checkout
      const checkoutResult = await checkOutGuest(currentGuest.requestId, currentGuest.id)
      ...
      try {
        ...
        console.error("Failed to send checkout notification:", e)
      }
      ...
      toast({
        ...
        description: "Guest checked out successfully! A confirmation and feedback link has been sent.",
      })
    } catch (error: any) {
                ...
                <div className="bg-blue-50 p-4 rounded-lg flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      You are about to check out <strong>{currentGuest.name}</strong> from <strong>{currentGuest.organization}</strong>.
                    </p>
                  </div>
                </div>

                <div className="text-sm text-gray-500 italic">
                  * A confirmation message and feedback link will be sent to the guest automatically.
                </div>

                <div className="flex gap-3">
                  <Button onClick={performCheckOut} className="flex-1 bg-blue-600 hover:bg-blue-700">
                    Confirm Checkout
                  </Button>
